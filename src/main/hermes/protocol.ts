...
const serverSchemas = [
  ...
  z.object({ type: z.literal('attachment.download.begin'), ...sequencedPayload.shape, payload: z.object({ attachment: z.object({ id: uuid, name: z.string().max(HERMES_MAX_LABEL_LENGTH), mime: z.string().max(HERMES_MAX_LABEL_LENGTH), kind: z.string().max(HERMES_MAX_LABEL_LENGTH), sizeBytes: z.number().int().min(0).max(HERMES_MAX_FILE_BYTES), sha256: z.string() }).strict() }).strict() }).strict(),
